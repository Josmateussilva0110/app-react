-- Adiciona filtro opcional de lista do mês (month_list) ao dashboard stats.

DROP FUNCTION IF EXISTS public.get_product_stats(integer, integer, uuid, text);
...
REVOKE ALL ON FUNCTION public.get_product_stats(integer, integer, uuid, text, boolean) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.get_product_stats(integer, integer, uuid, text, boolean) TO service_role;
GRANT EXECUTE ON FUNCTION public.get_product_stats(integer, integer, uuid, text, boolean) TO authenticated;

COMMENT ON FUNCTION public.get_product_stats(integer, integer, uuid, text, boolean) IS
  'Dashboard aggregates: monthly cards/charts + yearly evolution series. Optional month_list filter.';
