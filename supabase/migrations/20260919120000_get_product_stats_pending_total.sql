-- Acrescenta `pendingTotal` às estatísticas: a soma dos produtos não finalizados
-- dentro do mesmo recorte (período, escopo, usuário, month_list).
--
-- Motivo: a tela de lista precisava do total filtrado por status E das contagens
-- sem filtro, e isso custava duas chamadas a /products/stats — a mesma agregação
-- duas vezes. Com este campo o app faz uma chamada em 'todos' e deriva os três
-- totais: todos = total, pendente = pendingTotal, finalizado = total - pendingTotal.
--
-- Campo aditivo: quem não o consome continua funcionando igual.
...
BEGIN
  ...
  SELECT
    ...
    COALESCE(SUM(p.price) FILTER (WHERE NOT p.finished), 0)::float8 AS pending_total,
  ...
  RETURN jsonb_build_object(
    ...
    'pendingTotal', COALESCE(v_totals.pending_total, 0),
